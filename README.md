<!-- 
  Hi Marta! 
  
  Instructions:
  1. Replace 'your-github-username' with your actual GitHub username in all the image URLs.
  2. Fill in the URLs for your LinkedIn, portfolio, etc., in the 'Connect with Me' section.
  3. Customize the list of skills in the 'Tech Stack' section by adding or removing icons.
     Find more icons here: https://github.com/devicons/devicon/tree/master/icons
-->

<div id="header" align="center">
  <img src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExd3doMXEyeTdoczl0cTJzdmt3ajVhMWViNWE5Zm5uNXM0ZjExMjBoZiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/RnuOjf7cFoWTYL0c5M/giphy.gif" width="100"/>
  <h1>Howdy, I'm Marta! 👋</h1>
  <h3>Software Developer Intern at P4B 💻 | M.Sc. in Computer Science from UniPi 🎓</h3>
</div>

---

### 📖 About Me

<table width="100%">
  <tr>
    <td width="65%" valign="top" style="padding: 0 1.75rem 0 0;">
      <p>Welcome to my digital space! I'm a lifelong learner who recently graduated with a Master's degree in Computer Science from the University of Pisa (Italy).</p>
      <p>I'm currently applying my skills as a <strong>Software Developer Intern at Plan4Better</strong> in Munich (Germany), where I'm focused on building scalable and efficient software solutions.</p>
      <ul style="padding-left: 1.2rem; line-height: 1.6;">
        <li>🔭 Sharpening my backend wizardry one service at a time.</li>
        <li>🌱 Diving into cloud-native architectures and system design fundamentals.</li>
        <li>💬 Happy to chat about Quantum, Python, or my favorite anime arcs.</li>
        <li>📫 Reach me at <a href="mailto:m.locascio96@hotmail.com">m.locascio96@hotmail.com</a></li>
      </ul>
    </td>
    <td width="35%" valign="top" style="padding: 0 0 0 1.75rem; border-left: 1px solid rgba(88,166,255,0.2);">
      <div style="margin-bottom: 1.25rem;">
        <p align="left">
          <h3>🌍 Let's Connect! </strong>
          &nbsp;&nbsp;
          <a href="https://www.linkedin.com/in/96hoshi/" target="_blank">
            <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="LinkedIn" height="30" width="40" 
          </a>
        </p>
        </div>
      </div>
      <div style="margin-bottom: 1.25rem;">
        <h3>⭐ About Me</h3>
        <p style="line-height: 1.6;"> • 👾 Nerd by Nature • 🌸 Anime Addict 
        </p>
      </div>
      <div>
        <h3>🚀 Currently</h3>
        <ul style="padding-left: 1.2rem; line-height: 1.5;">
          <li>Building APIs with Python & RUST.</li>
          <li>Working on an open-source geo app.</li>
        </ul>
      </div>
    </td>
  </tr>
</table>

---

### 🛠️ My Tech Stack

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=c,cpp,python,java,ocaml,git,docker,mysql,postgres,linux&perline=5" />
  </a>
</p>

---

### 📈 My GitHub Stats
<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=96hoshi&theme=tokyonight" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=96hoshi&theme=tokyonight" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=96hoshi&theme=tokyonight" />
</p>
---

<p align="center">
  Yes, another ai-generated readme that no-one will ever read! 😉
</p>
